...
jQuery(document).ready(function(){
   ...
   var numero='<?php echo !empty($sol->ASP_SECUENCIAL) ? prepCampoMostrar($sol->ASP_SECUENCIAL) : null ; ?>';
...
$("#faspirante").validate({
       ...
       submitHandler: function(form){
           $.ajax({
               type: "POST",
               url:  "aspirante/admAspirante/"+accion,
               data: $("#faspirante").serialize(),
               dataType:"json",
               success: function(r){
                       if (r.cod>0) {
                           $("#faspirante").jConfirmacion({
                                        titulo:"Aspirante: "+r.numero,
                                        mensaje: r.mensaje,
