...
class Mcorreo extends CI_Model {
	...
	function mailCreaPersona($datos=null){			
				...
				$mail->SMTPAuth = true;
				$mail->SMTPSecure = "tls";
				$mail->setFrom(@$this->config->item('correoSistema'), @$this->config->item('usuarioSistema'));
				$mail->addReplyTo(@$this->config->item('correoSistema'), @$this->config->item('usuarioSistema'));
	...
	function mailConfirmaPersona($datos=null){			
				...
				$subject = utf8_decode('Confirmación, Made In BARTER');
				$pagina="<a style='color:#4257F9' href='".@$this->config->item('enlaceSistema')."'>"."www.madeinbarter.com</a>"."<br><br>";
					$mensaje = "<h3> Estimado/a, 
	...
	function mailConfirmaClave($datos=null){			
				...
				$subject = utf8_decode('Cambio de Clave, Made In BARTER');
				$pagina="<a style='color:#4257F9' href='".@$this->config->item('enlaceSistema')."'>"."www.madeinbarter.com</a>"."<br><br>";
					$mensaje = "<h3> Estimado/a, 
								<br>".utf8_encode($datos['nombresUsuario'])."<br><br>
	...
	function mailConfirmaAnula($datos=null){			
				...
				$subject = utf8_decode('Correo Anulación, Made In BARTER');
				$pagina="<a style='color:#4257F9' href='".@$this->config->item('enlaceSistema')."'>"."www.madeinbarter.com</a>"."<br><br>";
					$mensaje = "<h3> Estimado/a, 
								<br>".utf8_encode($datos['nombresUsuario'])."<br><br>
...
function mailPrueba(){
				...
				$mail->SMTPAuth = true;
				$mail->SMTPSecure = "tls";
				$mail->setFrom("madeinbarter@gmail.com","Made In Barter");
				$mail->addReplyTo("madeinbarter@gmail.com","Made In Barter");
				$mail->addAddress('darubahhp@gmail.com');	
				$subject = utf8_decode('Mail Prueba, Made In Barter');
...
function genEnlace($funcion=null){
         ...
         return $rutaSinCodificar."/persona/ruta/".$rutaACodificar;
      }
	
function mailReset($codigo,$resetclave=0){
	$sql = "SELECT USUARIO.*,TO_CHAR(SYSDATE,'DD-MM-YYYY-HH24:MI:SS') FECHA FROM USUARIO WHERE US_CODIGO=UPPER('".$codigo."')";
	$r=  $this->db->query($sql)->row_array();
	$funcion="barterenlace--".$r['US_CODIGO'];
	...
	if ($resetclave==1){
		...
		$subject = "Made In Barter reseto de Clave";
